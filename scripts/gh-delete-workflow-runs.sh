#!/usr/bin/env bash
#
# Bulk delete all GitHub Actions workflow runs from a repository.
# Usage: ./gh-delete-workflow-runs.sh [--dry-run] [--repo owner/repo]
#
...
Delete all GitHub Actions workflow runs from a repository.
...
Examples:
  $(basename "$0")                    # Delete all workflow runs
...
#######################################
# Fetch all workflow run IDs from the repository.
# Globals:
#   REPO
#   PER_PAGE
# Outputs:
...
fetch_run_ids() {
  ...
  while true; do
    response=$(gh api "repos/${REPO}/actions/runs?per_page=${PER_PAGE}&page=${page}" 2>/dev/null) \
      || response='{"workflow_runs":[]}'

    ids=$(echo "${response}" | jq -r '.workflow_runs[].id // empty' 2>/dev/null)
...
#######################################
# Delete workflow runs and report results.
# Globals:
#   REPO
# Arguments:
#   Run IDs array (passed by name)
#######################################
delete_runs() {
  local -n ids_ref=$1
  local deleted=0
...
main() {
  ...
  echo "Fetching workflow runs from ${REPO}..."
  ...
  if [[ "${DRY_RUN}" == true ]]; then
    echo "[DRY RUN] Would delete ${total} workflow run(s):"
  ...
  local confirm
  read -r -p "Delete ${total} workflow run(s) from ${REPO}? [y/N] " confirm
  if [[ ! "${confirm}" =~ ^[Yy]$ ]]; then
    echo "Aborted."
    exit 0
  fi
