...
// Run system pkg-config without prefix override
function runSystemPkgConfig(args) {
  try {
    const result = execSync(`pkg-config ${args}`, {
      encoding: 'utf8',
      stdio: ['pipe', 'pipe', 'pipe']
    });
    return result.trim();
...
if (mode === 'lib') {
  ...
  // Fallback: try system pkg-config with framework filtering
  const sysResult = runSystemPkgConfig(`--libs --static ${FFMPEG_LIBS}`);
  if (sysResult) {
    const filtered = process.platform === 'darwin' ? filterFrameworkFlags(sysResult) : sysResult;
    console.log(filtered);
    process.exit(0);
  }

  // Final fallback: let binding.gyp handle it
  process.exit(1);
  ...
  // Fallback: try system pkg-config
  const sysResult = runSystemPkgConfig(`--cflags-only-I ${FFMPEG_LIBS}`);
  if (sysResult) {
    console.log(sysResult.replace(/-I/g, '').trim());
    process.exit(0);
  }
