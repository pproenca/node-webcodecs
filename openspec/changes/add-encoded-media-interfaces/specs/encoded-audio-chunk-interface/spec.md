# Encoded Audio Chunk Interface

W3C WebCodecs spec section 8.1 - EncodedAudioChunk Interface

## ADDED Requirements

### Requirement: EncodedAudioChunk Internal Slots

The EncodedAudioChunk class SHALL maintain the following internal slots:
- **[[internal data]]**: An array of bytes representing the encoded chunk data
- **[[type]]**: Describes whether the chunk is a key chunk (EncodedAudioChunkType)
...
- **WHEN** an EncodedAudioChunk is constructed with valid init
- **THEN** all internal slots SHALL be populated from the init dictionary

#### Scenario: Duration slot is nullable
- **WHEN** an EncodedAudioChunk is constructed without duration in init
- **THEN** [[duration]] SHALL be null

---

### Requirement: EncodedAudioChunk Constructor

The EncodedAudioChunk constructor SHALL accept an EncodedAudioChunkInit dictionary and:
...
- **WHEN** constructor is called with valid EncodedAudioChunkInit
- **THEN** an EncodedAudioChunk instance SHALL be returned with correct attribute values
...
### Requirement: EncodedAudioChunk Type Attribute

The type attribute SHALL return the value of [[type]] slot.
The EncodedAudioChunkType enum values are "key" (for keyframes) and "delta" (for dependent frames).
...
### Requirement: EncodedAudioChunk Timestamp Attribute
...
### Requirement: EncodedAudioChunk Duration Attribute
...
- **WHEN** chunk is constructed with duration: 23220
- **THEN** duration attribute SHALL return 23220
...
### Requirement: EncodedAudioChunk ByteLength Attribute

The byteLength attribute SHALL return the value of [[byte length]] slot.

#### Scenario: ByteLength matches data size
- **WHEN** chunk is constructed with 5-byte data
- **THEN** byteLength attribute SHALL return 5
...
### Requirement: EncodedAudioChunk copyTo Method
...
### Requirement: EncodedAudioChunk Serialization

EncodedAudioChunk SHALL be Serializable with the following constraints:
...
### Requirement: EncodedAudioChunkInit Dictionary

The EncodedAudioChunkInit dictionary SHALL have the following members:
...
### Requirement: EncodedAudioChunkType Enum

The EncodedAudioChunkType enum SHALL have exactly two values:
- "key": Indicates the chunk can be decoded independently
- "delta": Indicates the chunk depends on previous frames for decoding
