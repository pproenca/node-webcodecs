...
async function testAudioDecoderQueue() {
    ...
    console.log(`Results: dequeueCount=${dequeueCount}, maxQueue=${maxQueueSize}, outputs=${audioDataOutputs.length}`);
...
async function testDecodeQueueSizeTracking() {
    ...
    assert.strictEqual(decoder.decodeQueueSize, 0, 'Initial queue size should be 0');
...
async function testResetClearsQueue() {
    ...
    assert.strictEqual(decoder.decodeQueueSize, 0, 'Queue size should be 0 after reset');
...
async function testKeyFrameRequirement() {
    ...
    assert.ok(errorReceived !== null, 'Error should be received for delta frame as first chunk');
    assert.ok(errorReceived.message.includes('key frame'), 'Error should mention key frame');
