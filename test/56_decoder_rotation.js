...
encoder.flush().then(() => {
    ...
    return decoder1.flush().then(() => {
        ...
        assert.strictEqual(decodedFrame.rotation, 90, 'Frame should have rotation=90');
        assert.strictEqual(decodedFrame.flip, false, 'Frame should have flip=false');
        decodedFrame.close();
        decoder1.close();
    ...
    return decoder2.flush().then(() => {
        assert(decodedFrame !== null, 'Should have decoded a frame');
        assert.strictEqual(decodedFrame.rotation, 0, 'Frame should have rotation=0');
        assert.strictEqual(decodedFrame.flip, true, 'Frame should have flip=true');
        decodedFrame.close();
        decoder2.close();
    ...
    return decoder3.flush().then(() => {
        assert(decodedFrame !== null, 'Should have decoded a frame');
        assert.strictEqual(decodedFrame.rotation, 180, 'Frame should have rotation=180');
        assert.strictEqual(decodedFrame.flip, true, 'Frame should have flip=true');
        decodedFrame.close();
        decoder3.close();
    ...
    try {
        ...
        assert(e.message.includes('rotation must be 0, 90, 180, or 270'),
    ...
    return decoder5.flush().then(() => {
        assert(decodedFrame !== null, 'Should have decoded a frame');
        assert.strictEqual(decodedFrame.rotation, 0, 'Default rotation should be 0');
        assert.strictEqual(decodedFrame.flip, false, 'Default flip should be false');
        decodedFrame.close();
        decoder5.close();
