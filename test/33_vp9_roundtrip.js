...
(async () => {
  ...
  console.log(
    `  ✓ Encoded ${NUM_FRAMES} frames to ${encodedChunks.length} chunks`,
  );

  assert(encodedChunks.length >= 1, 'Should have at least 1 encoded chunk');
  assert.strictEqual(
    encodedChunks[0].type,
    'key',
  ...
  assert.strictEqual(
    ...
    NUM_FRAMES,
    `Should decode ${NUM_FRAMES} frames`,
  );
  ...
  for (let i = 0; i < decodedFrames.length; i++) {
    ...
    assert.strictEqual(
      decoded.width,
      WIDTH,
      `Frame ${i} width should be ${WIDTH}`,
    );
    assert.strictEqual(
      decoded.height,
      HEIGHT,
      `Frame ${i} height should be ${HEIGHT}`,
    );
    assert.strictEqual(
      decoded.data.length,
      WIDTH * HEIGHT * 4,
      'Frame data size should match',
    );
  }
  ...
  for (let i = 0; i < decodedFrames.length; i++) {
    assert.strictEqual(
      typeof decodedFrames[i].timestamp,
      'number',
  ...
  for (let i = 0; i < firstDecoded.data.length; i += 4) {
    if (
      firstDecoded.data[i] > 0 ||
      firstDecoded.data[i + 1] > 0 ||
      firstDecoded.data[i + 2] > 0
    ) {
      nonZeroPixels++;
    }
  }
