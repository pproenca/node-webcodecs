...
test('close() marks frame as closed', () => {
    ...
    assert.ok(threwOnAllocationSize, 'allocationSize() should throw on closed frame');
...
test('clone() creates independent copy', () => {
    ...
    assert.strictEqual(cloned.codedWidth, original.codedWidth, 'clone codedWidth should match');
    assert.strictEqual(cloned.codedHeight, original.codedHeight, 'clone codedHeight should match');
    ...
    assert.strictEqual(cloned.codedWidth, WIDTH, 'clone should still be accessible after original closed');
    assert.strictEqual(cloned.timestamp, 5000, 'clone timestamp still accessible');

    // Clone can still provide allocation size
    const size = cloned.allocationSize();
    assert.strictEqual(size, FRAME_SIZE, 'clone allocationSize should still work');
...
test('allocationSize() returns correct size', () => {
    ...
    assert.strictEqual(actualSize, expectedSize, `allocationSize should be ${expectedSize} for 320x240 RGBA`);
...
test('copyTo() copies data correctly', async () => {
    ...
    assert.strictEqual(view[100], testValue, 'Middle byte should match fill value');
    assert.strictEqual(view[FRAME_SIZE - 1], testValue, 'Last byte should match fill value');
...
test('copyTo() with Uint8Array destination', async () => {
    ...
    assert.strictEqual(dest[FRAME_SIZE / 2], testValue, 'Middle data should match');
