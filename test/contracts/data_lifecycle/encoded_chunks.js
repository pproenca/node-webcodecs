...
const VIDEO_DATA = Buffer.from([0x00, 0x00, 0x00, 0x01, 0x67, 0x42, 0x00, 0x1e]);
...
test('EncodedVideoChunk has correct properties (key frame)', () => {
    ...
    assert.strictEqual(chunk.byteLength, VIDEO_DATA.length, `byteLength should be ${VIDEO_DATA.length}`);
});
...
test('EncodedVideoChunk with duration', () => {
    ...
    assert.strictEqual(chunk.byteLength, VIDEO_DATA.length, 'byteLength should match');
});
...
test('EncodedAudioChunk has correct properties (key frame)', () => {
    ...
    assert.strictEqual(chunk.type, 'key', 'type should be key');
    assert.strictEqual(chunk.timestamp, 0, 'timestamp should be 0');
    assert.strictEqual(chunk.byteLength, AUDIO_DATA.length, `byteLength should be ${AUDIO_DATA.length}`);
});
...
test('EncodedAudioChunk with duration', () => {
    ...
    assert.strictEqual(chunk.byteLength, AUDIO_DATA.length, 'byteLength should match');
});
