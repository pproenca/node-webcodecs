...
test('constructor creates audio with correct properties', () => {
    ...
    assert.strictEqual(audioData.sampleRate, SAMPLE_RATE, 'sampleRate should be 48000');
    assert.strictEqual(audioData.numberOfFrames, NUMBER_OF_FRAMES, 'numberOfFrames should be 1024');
    assert.strictEqual(audioData.numberOfChannels, NUMBER_OF_CHANNELS, 'numberOfChannels should be 2');
...
test('close() marks audio as closed', () => {
    ...
    assert.strictEqual(audioData.format, null, 'format should be null after close');
    ...
    assert.ok(threwOnAllocationSize, 'allocationSize() should throw on closed audio');
...
test('clone() creates independent copy', () => {
    ...
    assert.strictEqual(cloned.format, original.format, 'clone format should match');
    assert.strictEqual(cloned.sampleRate, original.sampleRate, 'clone sampleRate should match');
    assert.strictEqual(cloned.numberOfFrames, original.numberOfFrames, 'clone numberOfFrames should match');
    assert.strictEqual(cloned.numberOfChannels, original.numberOfChannels, 'clone numberOfChannels should match');
    assert.strictEqual(cloned.timestamp, original.timestamp, 'clone timestamp should match');

    // Close original
    original.close();

    // Clone should still be fully functional
    assert.strictEqual(cloned.format, FORMAT, 'clone should still be accessible after original closed');
    assert.strictEqual(cloned.sampleRate, SAMPLE_RATE, 'clone sampleRate still accessible');
    assert.strictEqual(cloned.timestamp, 10000, 'clone timestamp still accessible');
...
test('allocationSize() returns correct size', () => {
    ...
    assert.strictEqual(actualSize, expectedSize, `allocationSize should be ${expectedSize} for 1024-frame stereo f32`);
...
test('duration is computed correctly', () => {
    ...
    const expectedDuration = Math.floor((NUMBER_OF_FRAMES / SAMPLE_RATE) * 1000000);
    const actualDuration = audioData.duration;

    assert.strictEqual(actualDuration, expectedDuration, `duration should be ${expectedDuration} microseconds`);
...
test('copyTo() copies data correctly', () => {
    ...
    const destBuffer = new ArrayBuffer(NUMBER_OF_FRAMES * NUMBER_OF_CHANNELS * BYTES_PER_SAMPLE);
...
test('copyTo() throws on closed audio', () => {
    ...
    const dest = new ArrayBuffer(NUMBER_OF_FRAMES * NUMBER_OF_CHANNELS * BYTES_PER_SAMPLE);
    let threw = false;
    try {
...
test('close() is idempotent', () => {
    ...
    assert.strictEqual(audioData.format, null, 'Audio should remain closed after multiple close() calls');
});
