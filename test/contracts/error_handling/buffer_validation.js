const { VideoFrame, AudioData, EncodedVideoChunk, EncodedAudioChunk } = require('../../../dist');
...
test('VideoFrame rejects undersized buffer', () => {
    ...
    try {
        ...
        frame = new VideoFrame(Buffer.alloc(100), {
            codedWidth: 320, codedHeight: 240, timestamp: 0
        });
    } catch (e) {
        threw = true;
    }
    ...
    if (!threw && frame) {
        console.log('    (Note: native code does not validate buffer size on construction)');
    }
