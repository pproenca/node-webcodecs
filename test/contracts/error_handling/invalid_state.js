const { VideoEncoder, VideoDecoder, AudioEncoder, AudioDecoder, VideoFrame, AudioData } = require('../../../dist');
...
test('VideoEncoder.encode() on unconfigured throws', () => {
    ...
    const frame = new VideoFrame(Buffer.alloc(100 * 100 * 4), {
        codedWidth: 100, codedHeight: 100, timestamp: 0
    });
    let threw = false;
    try {
...
test('accessing closed VideoFrame throws or returns null', async () => {
    ...
    assert.ok(threwOrNull || threwAlloc || threwCopyTo, 'should throw or return null on closed frame');
});
...
test('accessing closed AudioData throws or returns null', () => {
    ...
    assert.ok(formatIsNull || threwClone || threwAlloc || threwCopyTo, 'should throw or return null on closed AudioData');
});
