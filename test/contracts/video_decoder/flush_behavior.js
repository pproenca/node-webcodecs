...
test('flush() emits buffered frames - encode frames first, then decode and flush', async () => {
    ...
    assert.ok(framesAfterFlush > 0, 'Should have emitted at least one frame after flush');
    assert.ok(framesAfterFlush >= framesBeforeFlush, 'Frame count should not decrease after flush');
