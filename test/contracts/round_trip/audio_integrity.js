...
function createAudioSamples(frameSize, sampleRate, numberOfChannels, frameIndex) {
...
test('encode-decode produces audio with correct sample rate', async () => {
    ...
    const samples = createAudioSamples(frameSize, sampleRate, numberOfChannels, 0);
    ...
    assert.ok(decodedAudioData.length > 0, 'Should have decoded at least one audio data');
...
test('encode-decode produces audio with correct channel count', async () => {
    ...
    for (const decoded of decodedAudioData) {
        assert.strictEqual(decoded.numberOfChannels, numberOfChannels,
...
test('timestamps are preserved through round-trip', async () => {
    ...
    for (let i = 0; i < frameCount; i++) {
        const samples = createAudioSamples(frameSize, sampleRate, numberOfChannels, i);
        const frameDurationMicros = Math.floor(frameSize / sampleRate * 1_000_000);
        const audioData = new AudioData({
            format: 'f32',
            sampleRate: sampleRate,
            numberOfFrames: frameSize,
    ...
    for (let i = 1; i < sortedDecoded.length; i++) {
        assert.ok(sortedDecoded[i] >= sortedDecoded[i - 1],
    ...
    assert.ok(sortedDecoded[0] >= 0, 'First decoded timestamp should be non-negative');
});
