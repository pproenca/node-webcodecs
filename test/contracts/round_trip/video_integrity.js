...
test('encode-decode produces frames with correct dimensions', async () => {
    ...
    for (const decoded of decodedFrames) {
        assert.strictEqual(decoded.width, width, `Decoded width should be ${width}`);
        assert.strictEqual(decoded.height, height, `Decoded height should be ${height}`);
    }
});
...
test('timestamps are preserved through round-trip', async () => {
    ...
    assert.strictEqual(decodedTimestamps.length, frameCount,
    ...
    assert.strictEqual(sortedDecoded[0], 0, 'First decoded timestamp should be 0');
});
...
test('multiple frames round-trip successfully', async () => {
    ...
    assert.strictEqual(decodedCount, frameCount,
