...
test('flush() emits buffered audio data - encode audio, decode, flush, verify output', async () => {
    ...
    assert.ok(audioAfterFlush > 0, 'Should have emitted at least one audio data after flush');
    assert.ok(audioAfterFlush >= audioBeforeFlush, 'Audio data count should not decrease after flush');
...
test('state remains configured after flush', async () => {
    ...
    assert.strictEqual(decoder.state, 'configured', 'State should be configured before flush');
    ...
    assert.strictEqual(decoder.state, 'configured', 'State should remain configured after flush');
