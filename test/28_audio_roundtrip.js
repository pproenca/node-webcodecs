const { AudioData, AudioEncoder, AudioDecoder, EncodedAudioChunk } = require('../dist');
...
async function main() {
    ...
    try {
        ...
        console.log(`✓ Created AudioData: ${inputAudio.numberOfFrames} frames, ${inputAudio.sampleRate}Hz, ${inputAudio.numberOfChannels}ch\n`);
        ...
        const encoder = new AudioEncoder({
            output: (chunk, metadata) => {
                ...
                console.log(`  Got encoded chunk: ${chunk.type}, ${chunk.byteLength} bytes`);
            },
            error: (e) => {
        ...
        const decoder = new AudioDecoder({
            output: (audio) => {
                ...
                console.log(`  Got decoded AudioData: ${audio.numberOfFrames} frames, format: ${audio.format}`);
            },
            error: (e) => {
        ...
        assert(decodedAudioData.length > 0, 'Should have decoded at least one AudioData');
        ...
        for (const audio of decodedAudioData) {
            ...
            assert.strictEqual(audio.sampleRate, sampleRate, 'Sample rate should match');
            assert.strictEqual(audio.numberOfChannels, numberOfChannels, 'Channel count should match');
        ...
        console.log(`  Total decoded frames: ${totalDecodedFrames} (input was ${numberOfFrames})`);
        assert(Math.abs(totalDecodedFrames - numberOfFrames) < tolerance,
        ...
        console.log(`  Opus decoded: ${opusDecoded.length} AudioData objects, ${totalOpusFrames} total frames`);
        assert(Math.abs(totalOpusFrames - 48000) < opusTolerance,
