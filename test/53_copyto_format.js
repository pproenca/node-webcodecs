...
assert.strictEqual(i420Size, width * height * 3 / 2, 'I420 allocation size should be w*h*1.5');
...
Promise.resolve(layout).then(layout => {
    ...
    const uPlane = i420Buffer.subarray(width * height, width * height + (width/2) * (height/2));
    const vPlane = i420Buffer.subarray(width * height + (width/2) * (height/2));
    ...
    assert(avgY > 50 && avgY < 100, `Y plane should have values around 76-82 for red, got ${avgY}`);
    ...
    assert(avgU > 70 && avgU < 110, `U plane should have values around 84-90 for red, got ${avgU}`);
    ...
    assert(avgV > 200, `V plane should have values around 240 for red, got ${avgV}`);
