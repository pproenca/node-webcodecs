...
async function testScenario1_Configure() {
    ...
    assert.strictEqual(encoder.state, 'configured', 'State should be configured after configure()');
...
async function testScenario2_EncodeFrame() {
    ...
    assert.strictEqual(receivedChunk.type, 'key', 'First chunk should be a keyframe');
...
async function testScenario3_FlushEncoder() {
    ...
    assert.ok(chunksAfterFlush >= chunksBeforeFlush, 'Flush should deliver remaining chunks');
...
async function testScenario4_CloseEncoder() {
    ...
    assert.strictEqual(encoder.state, 'closed', 'State should be closed after close()');
    console.log('   ✅ PASS');
}
...
async function testScenario5_ChunkProperties() {
    ...
    assert.ok(['key', 'delta'].includes(chunk.type), `type should be 'key' or 'delta', got '${chunk.type}'`);
    assert.strictEqual(typeof chunk.timestamp, 'number', 'timestamp should be a number');
    assert.ok(chunk.byteLength > 0, 'byteLength should be > 0');
    assert.ok(chunk.data instanceof Buffer || chunk.data instanceof Uint8Array, 'data should be a buffer');

    encoder.close();
    console.log(`   Chunk: type=${chunk.type}, ts=${chunk.timestamp}, size=${chunk.byteLength}`);
    console.log('   ✅ PASS');
}
...
async function testEndToEnd_WriteH264File() {
    ...
    console.log(`   ✅ Wrote ${chunks.length} chunks (${totalBytes} bytes) to ${OUTPUT_FILE}`);
