...
type EncodedVideoChunkCtor = WebCodecsModule['EncodedVideoChunk'];
type EncodedVideoChunkInstance = InstanceType<EncodedVideoChunkCtor>;

const CURRENT_DIR = path.dirname(fileURLToPath(import.meta.url));
const OUTPUT_FILE = path.join(CURRENT_DIR, '../../test-output-a1.h264');
...
async function testScenario1_Configure(VideoEncoder: VideoEncoderCtor): Promise<void> {
...
async function testScenario2_EncodeFrame(
  ...
  let receivedChunk: EncodedVideoChunkInstance | null = null;

  const encoder = new VideoEncoder({
    output: (chunk: EncodedVideoChunkInstance) => {
      chunkReceived = true;
      receivedChunk = chunk;
    },
    error: e => {
...
async function testScenario3_FlushEncoder(
  ...
  const chunks: EncodedVideoChunkInstance[] = [];

  const encoder = new VideoEncoder({
    output: (chunk: EncodedVideoChunkInstance) => chunks.push(chunk),
    error: e => {
      throw e;
    },
  });
...
async function testScenario4_CloseEncoder(VideoEncoder: VideoEncoderCtor): Promise<void> {
...
async function runAllTests(): Promise<void> {
  ...
  try {
    ...
    await testScenario1_Configure(VideoEncoder);
    await testScenario2_EncodeFrame(VideoEncoder, VideoFrame);
    await testScenario3_FlushEncoder(VideoEncoder, VideoFrame);
    await testScenario4_CloseEncoder(VideoEncoder);
    await testScenario5_ChunkProperties(VideoEncoder, VideoFrame);
    await testEndToEnd_WriteH264File(VideoEncoder, VideoFrame);
