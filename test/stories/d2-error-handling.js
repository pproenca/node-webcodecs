...
async function testScenario1_InvalidStateError() {
    ...
    assert.ok(threw || errorReceived, 'Should have thrown or called error callback');
    console.log('   ✅ PASS');
}
...
async function testScenario3_ResetRecovery() {
    ...
    console.log('   Then it returns to "unconfigured" state and can be reconfigured');
    ...
    if (typeof encoder.reset === 'function') {
        ...
        assert.strictEqual(encoder.state, 'configured', 'Should be configured after reconfigure');
        console.log('   ✅ PASS');
    } else {
...
async function testScenario5_TypeErrorOnInvalidConfig() {
    ...
    const invalidConfigs = [
        ...
        { name: 'negative width', config: { codec: 'avc1.42001E', width: -1, height: 480 } },
        { name: 'zero height', config: { codec: 'avc1.42001E', width: 640, height: 0 } },
    ];
