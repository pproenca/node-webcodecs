...
assert.strictEqual(
  tracks.length,
  1,
  'Static image should have exactly 1 track',
);

// Test ImageTrackList.selectedIndex
assert(
  typeof tracks.selectedIndex === 'number',
  'tracks.selectedIndex should be a number',
);
...
assert(
  typeof tracks.selectedTrack === 'object',
  'selectedTrack should be an object',
);
assert(
  typeof tracks.selectedTrack.animated === 'boolean',
  'animated should be a boolean',
);
assert.strictEqual(
  tracks.selectedTrack.animated,
  false,
  'Static PNG should not be animated',
);
assert(
  typeof tracks.selectedTrack.frameCount === 'number',
  'frameCount should be a number',
);
assert.strictEqual(
  tracks.selectedTrack.frameCount,
  1,
  'Static image should have 1 frame',
);
assert(
  typeof tracks.selectedTrack.repetitionCount === 'number',
  'repetitionCount should be a number',
);
assert.strictEqual(
  tracks.selectedTrack.repetitionCount,
  0,
  'Static image should have 0 repetitions',
);
assert.strictEqual(
  tracks.selectedTrack.selected,
  true,
...
assert.strictEqual(
  ...
  'tracks[0] should equal selectedTrack',
);

// Test dimension properties on selectedTrack
assert(
  typeof tracks.selectedTrack.width === 'number',
  'width should be a number',
);
assert(
  typeof tracks.selectedTrack.height === 'number',
...
assert.strictEqual(
  decoder.complete,
  true,
  'Static image should be immediately complete',
);
