...
async function testRoundTrip() {
  ...
  for (let i = 0; i < FRAME_COUNT; i++) {
    ...
    originalFrames.push({
      timestamp: i * 33333,
      avgR:
        rgba.filter((_, idx) => idx % 4 === 0).reduce((a, b) => a + b, 0) /
        (WIDTH * HEIGHT),
      avgG:
        rgba.filter((_, idx) => idx % 4 === 1).reduce((a, b) => a + b, 0) /
        (WIDTH * HEIGHT),
      avgB:
        rgba.filter((_, idx) => idx % 4 === 2).reduce((a, b) => a + b, 0) /
        (WIDTH * HEIGHT),
  ...
  assert(
    encodedChunks.length >= FRAME_COUNT,
    `Expected at least ${FRAME_COUNT} chunks, got ${encodedChunks.length}`,
  );
  ...
  const decoder = new VideoDecoder({
    output: frame => {
      ...
      decodedFrames.push({
        ...
        avgR:
          dest.filter((_, idx) => idx % 4 === 0).reduce((a, b) => a + b, 0) /
          (WIDTH * HEIGHT),
        avgG:
          dest.filter((_, idx) => idx % 4 === 1).reduce((a, b) => a + b, 0) /
          (WIDTH * HEIGHT),
        avgB:
          dest.filter((_, idx) => idx % 4 === 2).reduce((a, b) => a + b, 0) /
          (WIDTH * HEIGHT),
      });
      frame.close();
    },
  ...
  assert(
    decodedFrames.length >= FRAME_COUNT,
    `Expected at least ${FRAME_COUNT} decoded frames, got ${decodedFrames.length}`,
  );
  ...
  for (
    let i = 0;
    i < Math.min(originalFrames.length, decodedFrames.length);
    i++
  ) {
    ...
    if (i > 0) {
      assert(
        decoded.timestamp >= decodedFrames[i - 1].timestamp,
  ...
  console.log(
    `  Color match rate: ${(matchRate * 100).toFixed(1)}% (${matchCount}/${FRAME_COUNT})`,
  );

  // At least 80% of frames should have similar colors after lossy compression
  assert(
    matchRate >= 0.8,
    `Expected at least 80% color match, got ${(matchRate * 100).toFixed(1)}%`,
  );
