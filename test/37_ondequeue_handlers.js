...
const { VideoEncoder, VideoDecoder, AudioEncoder, AudioDecoder, VideoFrame, EncodedVideoChunk, AudioData, EncodedAudioChunk } = require('../dist');
...
// Test 1: VideoEncoder already has ondequeue
{
    ...
    let called = false;
    encoder.ondequeue = () => { called = true; };

    assert.strictEqual(typeof encoder.ondequeue, 'function', 'ondequeue should be settable');
...
// Test 2: VideoDecoder ondequeue
{
    ...
    assert.strictEqual(typeof decoder.ondequeue, 'function', 'ondequeue should be settable');
