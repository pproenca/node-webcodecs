...
asyncTest(
  '1.1 VideoEncoder emits metadata with decoderConfig.description',
  async () => {
    ...
    assert(
      metadataReceived.decoderConfig !== undefined,
      'decoderConfig should exist',
    );
    assert(
      metadataReceived.decoderConfig.description !== undefined,
...
asyncTest(
  '1.2 VideoEncoder actually encodes VP9 (not just H.264)',
  async () => {
    ...
    const isH264 =
      data[0] === 0 && data[1] === 0 && (data[2] === 0 || data[2] === 1);
...
asyncTest(
  '1.3 VideoEncoder.encodeQueueSize tracks pending operations',
  async () => {
    const encoder = new VideoEncoder({
      output: () => {},
      error: e => {
...
asyncTest(
  '1.4 VideoEncoder respects alpha: "keep" config',
  async () => {
    let outputChunk = null;
    let metadataReceived = null;
    ...
    if (metadataReceived && metadataReceived.alphaSideData) {
      ...
      throw new Error(
        'alphaSideData not present in metadata (alpha may be ignored)',
      );
    }
  },
  true,
); // Expected to fail based on code review
...
asyncTest(
  '2.1 VideoDecoder.decodeQueueSize tracks pending operations',
  async () => {
...
asyncTest('3.2 Opus encoder applies complexity option', async () => {
  ...
  const lowComplexityChunks = [];
  const lowEncoder = new AudioEncoder({
    output: chunk => {
      lowComplexityChunks.push(chunk);
    },
    error: e => {
      throw e;
    },
  ...
  const highComplexityChunks = [];
  const highEncoder = new AudioEncoder({
    output: chunk => {
      highComplexityChunks.push(chunk);
    },
    error: e => {
      throw e;
    },
  ...
  assert(
    lowComplexityChunks.length > 0,
    'Low complexity should produce chunks',
  );
  assert(
    highComplexityChunks.length > 0,
    'High complexity should produce chunks',
  );
...
asyncTest(
  '3.3 AudioEncoder emits metadata with decoderConfig',
  async () => {
    let metadataReceived = null;
    ...
    console.log(
      `  (decoderConfig: ${JSON.stringify(metadataReceived?.decoderConfig)})`,
    );
  },
  true,
...
asyncTest('4.1 VideoFrame.copyTo converts to requested format', async () => {
  ...
  console.log(
    `  (First Y bytes: ${yPlane[0]}, ${yPlane[1]}, ${yPlane[2]}, ${yPlane[3]})`,
  );
...
asyncTest(
  '4.2 VideoFrame.copyTo respects rect parameter',
  async () => {
    const frame = createTestFrame(320, 240, 0);
...
asyncTest('4.3 VideoFrame duration preserved through roundtrip', async () => {
  ...
  assert.strictEqual(
    frame.duration,
    originalDuration,
    'Original duration should match',
  );
...
asyncTest(
  '5.1 ImageDecoder.isTypeSupported returns Promise',
  async () => {
  ...
  true,
); // May fail if returns boolean sync

// Test 5.2: Does ImageDecoder handle animated GIF frames?
asyncTest(
  '5.2 ImageDecoder supports animated GIF frame index',
  async () => {
    ...
    console.log(
      `  (frameCount: ${decoder.tracks?.[0]?.frameCount || decoder.tracks?.selectedTrack?.frameCount})`,
    );
  ...
  true,
); // May fail if animation not supported
...
asyncTest(
  '6.2 VideoEncoder throws NotSupportedError for unsupported codec',
  async () => {
    const encoder = new VideoEncoder({
      output: () => {},
      error: () => {},
