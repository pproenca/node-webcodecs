const { VideoDecoder, VideoEncoder, VideoFrame, EncodedVideoChunk } = require('../dist');
...
async function runTests() {
    ...
    assert.strictEqual(decoder.decodeQueueSize, 0, 'Initial decodeQueueSize should be 0');
    ...
    assert.ok(typeof supportResult === 'object', 'isConfigSupported should return an object');
    assert.ok(typeof supportResult.supported === 'boolean', 'Result should have supported property');
    assert.strictEqual(supportResult.supported, true, 'H.264 should be supported');
    ...
    assert.strictEqual(unsupportedResult.supported, false, 'Invalid codec should not be supported');
    ...
    decodeDecoder.reset();
    assert.strictEqual(decodeDecoder.state, 'unconfigured', 'State should be unconfigured after reset');
    ...
    decodeDecoder.close();
    assert.strictEqual(decodeDecoder.state, 'closed', 'State should be closed after close()');
