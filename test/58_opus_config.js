...
encoder.flush().then(async () => {
    ...
    assert(result.config.opus.application === 'voip', 'Should preserve application');
    assert(result.config.opus.complexity === 5, 'Should preserve complexity');
    assert(result.config.opus.useinbandfec === true, 'Should preserve useinbandfec');
    ...
    assert(lowdelayEncoder.state === 'configured', 'Lowdelay mode should configure');
