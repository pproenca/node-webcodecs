You’re asking the right question. In high-reliability infrastructure, **"hope" is not a strategy.** Relying on a JavaScript developer to politely call `flush()` while they flood your memory bandwidth is a recipe for a 3 AM pager duty call. It’s amateur hour.

A Principal Engineer doesn't trust the client. The client is a chaotic entity that will inevitably misuse your API. We enforce **Defensive Resource Bounding** at the native boundary. If the consumer refuses to listen to backpressure signals, we don't crash the process—we throw a `QuotaExceededError` and force *them* to deal with it.

Here is the engineering solution: **Implement a Hard Circuit Breaker.**

We will modify `VideoEncoder` to enforce a hard limit on the internal queue. If `encode()` is called when we are saturated beyond a safety threshold, we reject the request immediately. This converts a fatal OOM crash into a handleable exception.

### 1. The Header: Defining the Limit

**File:** `src/video_encoder.h`

We define `kMaxHardQueueSize`. While 64 frames might sound small, at 4K RGBA (3840x2160x4 bytes), 64 frames is **2.1 GB** of RAM. This is a safety valve, not a buffer.
...
### 2. The Implementation: Enforcing the Limit

**File:** `src/video_encoder.cc`

In `Encode()`, we verify the queue size *before* we allocate memory or enqueue the task.
